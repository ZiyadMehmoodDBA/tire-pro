...
// ── Catalog Scraper Tab ────────────────────────────────────────────────────────
function CatalogScraperTab() {
  /* ── Config state ── */
  const [config, setConfig]     = useState<{ enabled: boolean; schedule: string; scheduleOptions: { value: string; label: string }[]; isRunning: boolean; isScheduled: boolean } | null>(null);
  ...
  /* ── Load config once ── */
  useEffect(() => {
    api.catalog.scraperConfig()
      .then(setConfig)
      .catch(e => setConfigErr(e.message));
  }, []);

  /* ── Poll status every 5 s while on this tab ── */
  const loadStatus = useCallback(() => {
    api.catalog.scraperStatus()
      .then(setStatus)
  ...
  const handleDelete = async (id: number) => {
    setDeletingId(id);
    try {
      await api.catalog.deleteEntry(id);
      loadEntries();
  ...
  return (
    <div className="space-y-8">
            ...
            <>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Periodically fetches tire data from <span className="font-medium text-slate-600">gtr.com.pk</span> and updates the catalog
                  </p>
                </div>
                <button
              ...
              <div>
                ...
                <select
                  ...
                  onChange={e => setConfig(c => c ? { ...c, schedule: e.target.value } : c)}
              ...
              <div className="text-xs text-slate-400 border-t border-slate-200 pt-3">
                ...
                &ensp;·&ensp;Products: BG ECONO, BG FALCON, BG LUXO PLUS, BG THUNDER MAX, BG MAX SPORT &amp; more
              </div>
            </>
          )}
        </div>
      ...
      <div>
        ...
        {statusErr && (
          <div className="flex items-center gap-2 text-sm text-red-700 bg-red-50 border border-red-100 rounded-xl px-4 py-2.5 mb-3">
            <AlertCircle size={14} /> {statusErr}
          </div>
        )}
              ...
              <tbody className="divide-y divide-slate-50">
                {status.recentLogs.map((log: any) => (
                  <tr key={log.id} className="hover:bg-slate-50/50">
                    <td className="px-3 py-2">
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${statusBadge(log.status)}`}>
                        {log.status}
                      </span>
                    </td>
      ...
      {/* ── Section 3: Catalog Data Browser ────────────────────────────── */}
      <div>
        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 flex items-center gap-1.5">
        ...
        <div className="bg-white border border-slate-100 rounded-2xl overflow-hidden">
          ...
          {entryTotalPages > 1 && (
            <div className="flex items-center justify-between px-4 py-2.5 border-t border-slate-100">
              ...
              <button
                onClick={() => setEntPage(p => Math.min(entryTotalPages, p + 1))}
                disabled={entPage === entryTotalPages}
                className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg disabled:opacity-30 transition-colors"
              >
